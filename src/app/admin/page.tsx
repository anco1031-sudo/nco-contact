  ...
  if (!user) {
    return (
      <div className="max-w-md mx-auto px-4 py-16">
        <div className="bg-card-bg rounded-2xl border border-border p-8 shadow-sm">
          ...
          <form onSubmit={handleLogin} className="space-y-4">
            <div>
              ...
              <input
                ...
                className="w-full px-4 py-2.5 border border-border rounded-lg text-sm focus:ring-2 focus:ring-primary/30 focus:border-primary outline-none"
              />
            </div>
      ...
      <div className="max-w-md mx-auto px-4 py-16 text-center">
        <div className="bg-card-bg rounded-2xl border border-border p-8 shadow-sm">
          ...
          <p className="text-muted text-sm mb-4">
            บัญชีของท่านไม่มีสิทธิ์ Administrator
          </p>
          <div className="flex gap-3 justify-center">
            <button
              onClick={handleLogout}
              className="px-6 py-2.5 border border-border rounded-lg text-sm font-medium hover:bg-gray-50 transition-colors flex items-center gap-2"
            >
              <LogOut size={16} />
              ออกจากระบบ
            </button>
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-8">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-8 gap-4">
        ...
        <button
          onClick={onLogout}
          className="px-4 py-2 border border-border rounded-lg text-sm font-medium hover:bg-gray-50 transition-colors flex items-center gap-2"
        >
          <LogOut size={16} />
          ออกจากระบบ
        </button>
      ...
      <div className="flex gap-2 mb-6 border-b border-border pb-2">
        ...
        >
          ...
          คำขอที่รออนุมัติ
          {pendingCount > 0 && (
            <span className="bg-accent text-primary text-xs px-2 py-0.5 rounded-full font-bold">
              {pendingCount}
            </span>
          )}
        </button>
          ...
          {loadingRequests ? (
            ...
            <div className="space-y-4">
              {requests.map((req) => (
                <div
                  key={req.id}
                  className={`bg-card-bg rounded-xl border p-5 shadow-sm ${
                    req.status === "pending"
                      ? "border-accent/30"
                      : "border-border"
                  }`}
                >
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 text-sm mb-3">
                    <div>
                      <span className="text-muted">เหล่า: </span>
                      {req.unit}
                    </div>
                    <div>
                      <span className="text-muted">กองร้อย: </span>
                      {req.company}
                    </div>
                    {req.workplace && (
                      <div>
                        <span className="text-muted">ที่ทำงาน: </span>
                        {req.workplace}
                      </div>
                    )}
                    {req.phone && (
                      <div>
                        <span className="text-muted">เบอร์: </span>
                        {req.phone}
                      </div>
                    )}
                    {req.line_id && (
                  ...
                  <div className="text-xs text-muted border-t border-border pt-3">
                    ผู้ขอ: {req.requester_name} | เบอร์ผู้ขอ:{" "}
                    {req.requester_phone}
                  </div>
                  ...
                  {req.status === "pending" && (
                    <div className="flex gap-2 mt-4">
                      ...
                      <button
                        ...
                        className="px-3 py-2 border border-border rounded-lg text-sm text-muted hover:bg-gray-50 transition-colors"
                      >
                        <Trash2 size={16} />
                      </button>
                    </div>
...
function ContactManager() {
  const [contacts, setContacts] = useState<
    { id: string; rank: string; first_name: string; last_name: string; unit: string; company: string }[]
  >([]);
  const [loading, setLoading] = useState(true);
  ...
  async function fetchContacts() {
    ...
    const { data } = await supabase
      .from("contacts")
      .select("id, rank, first_name, last_name, unit, company")
      .order("rank");
...
function AddContactForm({ onAdded }: { onAdded: () => void }) {
  ...
  return (
    <form
      ...
      className="bg-card-bg rounded-xl border border-accent/30 p-5 shadow-sm space-y-4"
    >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
        <select
          ...
          required
          className="px-3 py-2 border border-border rounded-lg text-sm focus:ring-2 focus:ring-primary/30 outline-none"
        >
