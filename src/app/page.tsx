...
export default function HomePage() {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      {/* Search & Filter bar */}
      <div className="bg-card-bg rounded-xl shadow-sm border border-border p-4 mb-6">
        <div className="flex flex-col sm:flex-row gap-3">
          ...
          <div className="relative flex-1">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2.5 border border-border rounded-lg focus:ring-2 focus:ring-primary/30 focus:border-primary outline-none transition-all text-sm"
            />
            {searchTerm && (
              <button
          ...
          <button
            ...
            className={`flex items-center gap-2 px-4 py-2.5 rounded-lg text-sm font-medium border transition-colors ${
              showFilters || hasActiveFilters
                ...
                : "border-border hover:bg-gray-50 text-muted"
            }`}
          >
            <Filter size={16} />
            กรองข้อมูล
            {hasActiveFilters && (
              <span className="bg-accent text-primary text-xs px-1.5 py-0.5 rounded-full font-bold">
                !
              </span>
            )}
          ...
          <div className="mt-4 pt-4 border-t border-border grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
            <div>
              ...
              <select
                ...
                className="w-full px-3 py-2 border border-border rounded-lg text-sm focus:ring-2 focus:ring-primary/30 focus:border-primary outline-none"
              >
      ...
      {loading ? (
        ...
        <>
          {/* Desktop table */}
          <div className="hidden md:block bg-card-bg rounded-xl border border-border overflow-hidden shadow-sm">
                ...
                <tbody>
                  {filteredContacts.map((contact) => (
                    ...
                    >
                      ...
                      <td className="px-4 py-3 text-muted">{contact.unit}</td>
          ...
          <div className="md:hidden space-y-3">
            {filteredContacts.map((contact) => (
              <div
                key={contact.id}
                className="bg-card-bg rounded-xl border border-border p-4 shadow-sm"
              >
                <div className="flex items-start justify-between mb-2">
                  ...
                  <button
                    onClick={() =>
                      setExpandedRow(
                        expandedRow === contact.id ? null : contact.id
                      )
                    }
                ...
                <div className="text-xs text-muted space-y-1">
                  <p>
                    เหล่า: <span className="text-foreground">{contact.unit}</span>
                  </p>
                  <p>
                    กองร้อย:{" "}
                  ...
                  <div className="mt-3 pt-3 border-t border-border text-xs space-y-2">
                    ...
                    <div className="flex gap-2 pt-1">
                      {contact.phone && (
                        <a
                          href={`tel:${contact.phone}`}
                          className="inline-flex items-center gap-1 px-3 py-1.5 bg-green-50 text-green-700 rounded-md hover:bg-green-100 transition-colors font-medium"
                        >
                          <Phone size={14} />
                          {contact.phone}
                        </a>
